...
require_once $_SERVER['DOCUMENT_ROOT'] . '/CafeteriaWeb2/modelo/vo/Usuario.php';
    ...
    public function insert(Usuario $usuario) {
        try {
            $sql = "INSERT INTO usuario (nome,login,email,senha,foto)"
                    . "VALUES (:nome,:login,:email,:senha,:foto)";
    ...
    public function update($usuario) {
        try {
            $sql = "UPDATE usuario SET nome=:nome, login=:login,"
                    . "email=:email, foto=:foto, senha=:senha "
                    . "where id=:id";
            //perceba que na linha abaixo vai precisar de um import
            $p_sql = BDPDO::getInstance()->prepare($sql);
            $p_sql->bindValue(":nome", $usuario->getNome());
            ...
            $p_sql->bindValue(":senha", $usuario->getSenha());
            $p_sql->bindValue(":foto", $usuario->getFoto());
            //iremos critografar a senha para md5, asism o usuário terá mais segurança, já que frequentemente usamos a mesma senha
